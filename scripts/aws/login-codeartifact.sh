...
command_aws="aws codeartifact login --tool npm --repository ${env_vars["AWS_REPOSITORY"]} --domain ${env_vars["AWS_DOMAIN"]} --domain-owner ${env_vars["AWS_DOMAIN_OWNER"]} --region ${env_vars["AWS_REGION"]} --profile ${env_vars["AWS_PROFILE"]}"
